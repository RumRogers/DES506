...
namespace Player
{
    ...
    public class PlayerEntity : GameCore.Rules.MutableEntity
    {
        ...
        //Player Entity
        PlayerEntityProperties m_playerEntityProperties;
        ...
        public bool IsColliding()
        {
            ...
            {
                //Debug.DrawLine(transform.position, transform.position + (new Vector3(m_velocity.x, 0.0f, m_velocity.z).normalized * m_playerCollider.bounds.extents.x * m_collisionRayLengthMultiplyer));
            ...
            Vector3 horizontalRaySpacing = Vector3.Cross(rayDirection, transform.up);    // get perpendicular vector to our direction
            ...
            if (m_velocity.z != 0)
            {
                //rayStart = transform.position + ((m_playerCollider.bounds.extents.z /2) * rayDirection);
            ...
            horizontalRaySpacing -= horizontalRaySpacing * m_skinWidth;

            rayStart -= (verticalRaySpacing * (m_numHorizontalRays/ 2)) + ((horizontalRaySpacing) * (m_numVerticalRays / 2));
            ...
            for (int x = 0; x < m_numHorizontalRays; ++x)
            {
                for (int y = 0; y < m_numVerticalRays; ++y)
                {
                    ...
                    //Debug.DrawLine(rayStart, (rayStart + (rayDirection * m_rayLength)));
